...
void level_order_insert( struct node* root, int* arr, int start, int size)
{
    int left = 2*start+1;
    int right = 2*start+2;
    ...
    if(root->lchild == NULL && root->rchild == NULL) {
        if(left < size)
            root->lchild = createNode(arr[left]);
        if(right < size)
            root->rchild = createNode(arr[right]);
    }
    level_order_insert(root->lchild, arr, left, size);
    level_order_insert(root->rchild, arr, right, size);
